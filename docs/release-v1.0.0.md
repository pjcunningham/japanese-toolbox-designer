...
- **Kinematic Sliding Lid & Captured Locking Wedge:** Mathematically rigorous sliding lid mechanics calculating pocket depths ($R - I - T$), locked overlaps ($O$), release travel ($D$), clearance margins, and compound wedge geometry ($2^\circ$ plan taper $\alpha$, $10^\circ$ vertical undercut retention bevel $\beta$).
...
- **Local Persistence & JSON Interchange:** Browser `localStorage` multi-design lifecycle management (New, Save, Open, Rename, Duplicate, Delete) with legacy Schema V1 automatic in-memory migration, and portable single-design JSON import/export with ID conflict resolution.
...
- **Storage:** Browser `localStorage` (`jtd.designs.v1`, Schema V2)
- **Test Suite:** 293 Vitest unit/component tests, 36 Playwright E2E tests (100% passing across Chromium, Firefox, and WebKit)
